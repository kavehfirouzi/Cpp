...
#include <fstream>
extern std::ofstream f7;
#define FATIGUE_BUFFER 209
/* Hamming window */
...
float32_t EMG_Buffer_1[FATIGUE_BUFFER];														 // Quad Emg buffer to delay the it to take care of shifts for fatigue
...
void pWelch_quad(float newData, uint32_t* mean_freq, int start, int init)
{
	...
	if (init == 0)                                                                            // Not initializing
	{
			...
			switch (welch_state)
			{
				...
				case 1:
					length++;
					alaki = EMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_1];
					alaki2 = p*c_1;
					p1 = coeffs_real_1;
				    p2 = coeffs_img_1;
					ii = 9;
					...
					{
							p1[0] +=  alaki*Cosine(alaki2*ii);	// Real part of the coefficient
							p2[0]  -= -alaki*Sine(alaki2*ii);	// Imaginary part of the coefficient
							p1[1] +=  alaki*Cosine(alaki2*(ii+1));	// Real part of the coefficient
							p2[1]  -= -alaki*Sine(alaki2*(ii+1));	// Imaginary part of the coefficient
							p1[2] +=  alaki*Cosine(alaki2*(ii+2));	// Real part of the coefficient
							p2[2]  -= -alaki*Sine(alaki2*(ii+2));	// Imaginary part of the coefficient
							p1[3] +=  alaki*Cosine(alaki2*(ii+3));	// Real part of the coefficient
							p2[3]  -= -alaki*Sine(alaki2*(ii+3));	// Imaginary part of the coefficient
							p1 += 4;
							p2 += 4;
							ii += 4;
					}
					...
					{
						alaki = EMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_2];
						alaki2 = p*c_2;
						p1 = coeffs_real_2;
						p2 = coeffs_img_2;
						ii = 9;
					...
					if (start == 0)																																		// End of segment, average the power spectrums, calculate mean frequency and reset the state machine parameters
					{
						//if (length > 64)
						{
							...
							f2 = 125;
							pow2 = 0;
							for (i = 0; i < 24; i++)                                                      // for each coefficient, add the final two absolute values to the sum and find the average
							{
								c_1 = 0;
								c_2 = 0;
								flag = 0;
							...
							if (length > 64)
							{
								sum = sum - buff[pnt] +  y1 / pow1;																								// A moving average window of size 30 over the calculated mean frequencies
								buff[pnt] = y1 / pow1;
								//*mean_freq = y / pow;																													// For debug
								pnt++;
								if (pnt == average_size)																												// Reseting the pointer inside the buffer used for moving average
									pnt = 0;
								averager++;
								if (averager < average_size)																										// If less than average_size number of mean frequencies have been calculated so far, send 255 over bluetooth
									*mean_freq = const_val;
								else if (averager == average_size)																							// If 30 mean frequencies have been calculated, calculate their average
									 mid_point = sum / average_size;																							// This average is used as a point of compariosn fron bow on
								else
									*mean_freq = (sum / average_size) - mid_point + center;                       // Center the averaged mean frequencies around 15, based on the mid_point value calculated above
								//f7 << y1/pow1 << "\t" << y2/pow2 << std::endl;// Go to reset state
							}
						}
						welch_state = 0;
						length = 0;
...
/**
 * @brief	Exactly the same as pWelch_quad, just using different buffers. Performs pwelch power spectrum estimation and mean frequency calculation
 * @param newData	  : new filtered EMG data
 * @param mean_freq : pointer to the mean frequency
 * @param start   	: marks the start and stop of the calculation of the power spectrum and mean frequency
 * @return	nothing
	...
	if (init == 0)                                                                            // Not initializing
	{
			...
			switch (welch_state)
			{
				case 0:																																							// reset state
					...
					break;
				case 1:
					for (i = 0; i < nfft; i++)																												// with each incoming data, update all the coefficients
					{
								coeffs_real_1[i] +=  EMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_1]*Cosine(p*i*c_1);	// Real part of the coefficient
								coeffs_img_1[i]  -= -EMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_1]*Sine(p*i*c_1);	// Imaginary part of the coefficient
					}
					c_1++;																																						// Increment the counter
					if (c_1 == overlap + 1)
						flag =  1;																																			// This means we have received 32 samples, it's time to start the concurrent calculation for overlaping windows
					...
					if (flag == 1) 																																		// Starting concurrent calculation for overlaping windows
					{
						for (i = 0; i < nfft; i++)
						{
							coeffs_real_2[i] +=  EMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_2]*Cosine(p*i*c_2);		// Real part of the coefficient
							coeffs_img_2[i]  -= -EMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_2]*Sine(p*i*c_2);		// Imaginary part of the coefficient
						}
						c_2++;																																					// Increment the counter
						if (c_2 == SIZE)
						{
					...
					if (start == 0)																																		// End of segment, average the power spectrums, calculate mean frequency and reset the state machine parameters
					{
						...
						sum = sum - buff[pnt] +  y / pow;																								// A moving average window of size 30 over the calculated mean frequencies
						buff[pnt] = y / pow;
						//*mean_freq = y / pow;																													// For debug
						pnt++;
						if (pnt == average_size)																												// Reseting the pointer inside the buffer used for moving average
...
/**
 * @brief	finds the Sine value of an input in degrees
 */	
float Sine(float Input)
{
	int r;
	...
	else if (reminder <= PI)
	{
		reminder -= PI/2;
		r = (reminder/PI)*1000;
		return Sin_LUT[500-r];
	}
	else if (reminder <= (float)1.5*PI)
	{
		reminder -= PI;
		r = (reminder/PI)*1000;
		return -1*Sin_LUT[r];
	...
	{
		reminder -= (float)1.5*PI;
		r = (reminder/PI)*1000;
		return -1*Sin_LUT[500-r];
	}
}

/**
 * @brief	finds the Cosine value of an input in degrees
 */
float Cosine(float Input)
{
	int r;
