...
float32_t kEMG_Buffer_1[FATIGUE_BUFFER];														 // Quad Emg buffer to delay the it to take care of shifts for fatigue
float32_t kEMG_Buffer_2[FATIGUE_BUFFER];														 // Ham Emg buffer to delay the it to take care of shifts for fatigue
...
void pWelch_quad2(float newData, uint32_t* mean_freq, int start, int init)
{
	...
	{
			kEMG_Buffer_1[EMGB_ptr1_1] = newData;																			
			EMGB_ptr1_1++;
			EMGB_ptr1_2++;
			...
			switch (welch_state)
			{
				...
				case 1:
					length++;
					temp1 = kEMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_1];
					temp2 = p*c_1;
					p1 = coeffs_real_1;
				    p2 = coeffs_img_1;
				    h = 9;
					...
					{
							p1[0] +=  temp1*cos(temp2*h);																						// Real part of the coefficient
							p2[0]  -= temp1*sin(temp2*h);																							// Imaginary part of the coefficient
							p1[1] +=  temp1*cos(temp2*(h+1));	
							p2[1]  -= temp1*sin(temp2*(h+1));	
							p1[2] +=  temp1*cos(temp2*(h+2));	
							p2[2]  -= temp1*sin(temp2*(h+2));	
							p1[3] +=  temp1*cos(temp2*(h+3));	
							p2[3]  -= temp1*sin(temp2*(h+3));	
							p1 += 4;
							p2 += 4;
							h += 4;
					}
					...
					{
						temp1 = kEMG_Buffer_1[EMGB_ptr1_2]*Hamming[c_2];
						temp2 = p*c_2;
						p1 = coeffs_real_2;
						p2 = coeffs_img_2;
						h = 9;
...
void pWelch_ham2(float newData, uint32_t* mean_freq, int start, int init)
{
  ...
  float f1, pow1, f2, pow2;
	...
	if (init == 0)                                                                            // Not initializing
	{
			kEMG_Buffer_2[EMGB_ptr1_1] = newData;																			
			EMGB_ptr1_1++;
			EMGB_ptr1_2++;
			if (EMGB_ptr1_1 == FATIGUE_BUFFER-1)
				EMGB_ptr1_1 = 0;
			...
			switch (welch_state)
			{
				...
				case 1:
					length++;
					temp1 = kEMG_Buffer_2[EMGB_ptr1_2]*Hamming[c_1];
					temp2 = p*c_1;
				  p1 = coeffs_real_1;
				  p2 = coeffs_img_1;
				  h = 9;
					...
					if (flag == 1) 																																		// Starting concurrent calculation for overlaping windows
					{
						temp1 = kEMG_Buffer_2[EMGB_ptr1_2]*Hamming[c_2];
						temp2 = p*c_2;
						p1 = coeffs_real_2;
						p2 = coeffs_img_2;
						h = 9;
					...
					if (start == 0)																																		// End of segment, average the power spectrums, calculate mean frequency and reset the state machine parameters
					{
						...
						if (length > 64)
						{
							sum = sum - buff[pnt] +  y1 / pow1;																						// A moving average window of size 30 over the calculated mean frequencies
							buff[pnt] = y1 / pow1;
							MeanFreq_H = (y2/ pow2)*256;																									// conveting to 2-byte fixed-point. normalize by 256 (max frequency) multiply by 2^16
							pnt++;
							if (pnt == average_size)																											// Reseting the pointer inside the buffer used for moving average
								pnt = 0;
							averager++;
