		...
		if ( (counter & 0x1) != 0 || counter >= 125)
		{
			...
			if (quad_detected == 1)
			{
				...
				if (reference >= 0)
					...
					reference = 0;
				shift_x = PosQBuffX[14] - BottX;
				shift_y = PosQBuffY[14] - BottY;
				g = shift_x * (long long)(TopX - BottX) + shift_y * (long long)(TopY - BottY);
				if (reference != 0)
					g = g / reference;
				g = reference - g;
				
				if (reference != 0 && cadence_acc != 0)
				{
					if (PosQBuffVel[14] < 0)
					{
						f6 << (g)/reference*180 << std::endl;
						k = (g)/reference*180;
					}
					else
					{
						f6 << 360 - (g)/reference*180 << std::endl;
						k = 360 - (g)/reference*180;
					}
				}
				else
				{
			...
			if (ham_detected == 1)
			{
				...
				if (reference >= 0)
					...
					reference = 0;
				//reference = sqrt(reference);
				shift_x = PosHBuffX[14] - BottX;
				shift_y = PosHBuffY[14] - BottY;
				g = shift_x * (long long)(TopX - BottX) + shift_y * (long long)(TopY - BottY);
				if (reference != 0)
					g = g / reference;
				//g = g / reference;
				...
				{
					if (PosHBuffVel[14] < 0)
						f6_2 << (g)/reference*180 << std::endl;
					else
						f6_2 << 360 - (g)/reference*180 << std::endl;
				}
				else
				{
					f6_2 << 0 << std::endl;
...
void PositionX(int x, int *pos, int init)
{
	...
	if (init == 0)
	{
		...
		{
		case 0:
			if (x > 0 && cadence_acc != 0)
				state = 1;
			break;
		case 1:
			if (x < 0 && (max - min) > 5000 && cadence_acc != 0)
				state = 2;
			else
					state = 0;
			break;
...
void PositionY(int y, int *pos, int init)
{
	...
	if (init == 0)
	{
		...
		{
			case 0:
				if (y > 0 && cadence_acc != 0)
					state = 1;
				break;
			case 1:
				if (y < 0 && (max - min) > 10000 && cadence_acc != 0)
					state = 2;
				else
					state = 0;
				break;
