...
void SpinScanGyroAlligned(uint32_t* rawData, int init)
{
	...
	static float delayBuffAngle[GYRO_DELAY];
	...
	if (init == 0 && downsampler == 0)
	{
		...
		if (ptr4 == GYRO_DELAY)
			ptr4 = 0;
		//////////////////////////////////
		gyroAngleZ += ((float)raw_z.raw_data/32.8*(float)0.008);
		gyroAngleY += ((float)raw_y.raw_data/32.8*(float)0.008);
		gyroAngleX += ((float)raw_x.raw_data/32.8*(float)0.008);
		delayBuffAngle[ptr3] = gyroAngleZ;
			...
			switch(state)
			{
				...
				case 1:
					...
					if (sz_prev < 0 && delayBuff[ptr4] > 0)
					{
						...
						{
							AllignGyroAcc(gyroSVDBuff, angleSVDBuff, alligned, alligncounter);
