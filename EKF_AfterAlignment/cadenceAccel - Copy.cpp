...
#define THRESH     1700
#define Accel_size 250																					// Size of the classification window
#define SMOOTH_SIZE 19																					// Size of the sliding window size used for smoothing
...
void Peakdetection_acc(int in, int* out);
...
//	static int dummy_2[SMOOTH_SIZE];
	static int ptr_1 = 0;
	//static int ptr_2 = 0;
  ...
  static int cadence;
	...
	{
			raw.raw_data = 0;
			raw.data_byte[0] = rawData[0];															// Converting the 2-byte, big-endian accelerometer value to 2-byte small-endian
			raw.data_byte[1] = rawData[1];
			...
			{
				raw.data_byte[2] = 0xFF;
				raw.data_byte[3] = 0xFF;
			}
																							
			//Smooth_1(raw.raw_data, &s_1, 0);														// Two rounds of snoothing by means of sliding windows. Instead of shifting values inside the buffer, circular poiters are used
		//	Smooth_2(s_1, &s_2, 0);
			/////////////Filtering
				sum_1 = sum_1 - dummy_1[ptr_1] + raw.raw_data;
				dummy_1[ptr_1] = raw.raw_data;
			...
			smoothedAccBuff[Accel_size-1] = s_2;												// Adding new smoothed accelerometer to the buffer
			if (s_2 > (max+min)/3)   																		// Classification of accelerometer values
			{
				Class=0;
			}
			else
...
void Peakdetection_acc(int in, int* out)
{
	...
	switch(state)																									// State machine
	{
		...
		case 1:
			...
			if (in == 0)																							// Segment has ended, go back to state 0
				state = 0;
			break;
	}
}
