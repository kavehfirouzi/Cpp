...
void PreProcessing(int RawEMG, int RawEMG2, float* FilteredEMG, float* SmoothedRMS, float* Detrended, int sel, int action)
{
		...
		double alaki;
			...
			if (action != 4)
			{
				if (sel == 0)
				{
					Moving_Average_HIGH(&(RawEMG), &filter_out, 0, 0);
					ButterQ(filter_out, &alaki);
					butter_out = abs(alaki);
					f1 << alaki << std::endl;
					Moving_Average_HIGH(&RawEMG2, &filter_out, 0, 2);
					ButterVL(filter_out, &alaki);
					butter_out += abs(alaki);
					f2 << abs(alaki) << std::endl;
				}
				else
				{
...
{
	static double DataBuff[5+1];
	static double OutBuff[4+1];
	double sum;
	int j;
	double Data = newData;
	for (j=0 ; j< 4; j++)
