...
extern std::ofstream f1, f2, f3, f4, f5, f6, f6_1, f6_2, X, Y, f8, f9, allign, unalligned;
...
void SpinScanGyroAlligned2(uint32_t* rawData, int init)
{
	...
	static int start = 0;
	int repstart, repend, repmiddle, quadstartmid, quadendmid;
	static int alaki = 0;
	float alakibuff[200];
	...
	if (init == 0 && downsampler == 0)
	{
		...
		////////////////////////////////////////////////////////////////
		{
			...
			if (start == 1)
			{
			  ...
			  {
				 if (quadStartLength < repmiddle)
					quadStartLength = abs(quadStartLength - repstart);
				 else
				 {
					quadStartLength = abs(quadStartLength - repmiddle);
					quadstartmid = 1;
				 }
			  }
			  else
			  ...
			  {
				 if (quadEndLength < repmiddle)
					quadEndLength = abs(quadEndLength - repstart);
				 else
				 {
					quadEndLength = abs(quadEndLength - repmiddle);
					quadendmid = 1;
				 }
			  }
			  else
			  ...
			  if (quad_detected_delayed == 1)
			  {
				...
				{
					if (quadstartmid == 0)
						angle  = (float)quadStartLength/abs(repmiddle-repstart)*180;
					else
						angle  = (float)quadStartLength/abs(repend - repmiddle)*180 + 180;
					if (angle >= 360)
						angle = 0;
				}
				else
				...
				{
					if (quadendmid == 0)
						angle  = (float)quadEndLength/abs(repmiddle-repstart)*180;
					else
						angle  = (float)quadEndLength/abs(repend - repmiddle)*180 + 180;
					if (angle >= 360)
					if (angle >= 360)
						angle = 0;
				}
