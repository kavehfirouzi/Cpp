...
extern struct MyBuffers *Buffers;
...
#include <fstream>
extern std::ofstream f1_1, f2, f3, f4, f5, ftest;

struct EKF_Buffer *EKF_Quad;
struct EKF_Buffer *EKF_Ham;
//void EKF_quad(float RMS, float *Sigma, float *RMS_out, int init)
//{
//	int ptr5;
//	float mean;
//	const float A[9] = {A_1, A_2, A_3, A_4, A_5, A_6, A_7, A_8, A_9};								// A matrix
//	float Bneg[9] = {-B1_1, 0, 0, 0, -B1_5, 0, 0, 0, -B1_9};					// -B							// Sk
//	float tempMAT_1[18];
//	float tempMAT_2[18];
//	float tempMAT_3[18];
//	float Q[36];
//	float R[18];
//	float Rinv_1[9];
//	float Rinv_2[9];                    // initial value of Ck
//	float Dinv  = 1;					 							// initial state values
//	const float zeros_1[3] = {0,0,0};
//	float zeros_2[4] = {0,0,0,1};
//	float max = -1000000;
//	float min = +1000000;
//	float observ;
//	int i, k;
//	if (init == 0)
//	{
//		EKF_Quad->Delay[EKF_Quad->ptr3] = RMS;
//		EKF_Quad->sum = EKF_Quad->sum - EKF_Quad->RMSBuff[EKF_Quad->ptr] + EKF_Quad->Delay[EKF_Quad->ptr4];											                      // Finding the mea
//		mean = EKF_Quad->sum / SMOOTHWINDOW;
//		EKF_Quad->RMSBuff[EKF_Quad->ptr] =  EKF_Quad->Delay[EKF_Quad->ptr4];
//		(EKF_Quad->ptr3)++;
//		if (EKF_Quad->ptr3 == 95)                    									// (95 = (251 - 61) - (251/2 - 51/2);
//			EKF_Quad->ptr3 = 0;
//		(EKF_Quad->ptr4)++;
//		if (EKF_Quad->ptr4 == 95)
//			EKF_Quad->ptr4 = 0;
//		k = EKF_Quad->ptr + (SMOOTHWINDOW-1)/2;
//		if (k>SMOOTHWINDOW-1)
//			k -= SMOOTHWINDOW;
//		(EKF_Quad->ptr)++;
//		if (EKF_Quad->ptr == SMOOTHWINDOW)
//			EKF_Quad->ptr = 0;
//
//		EKF_Quad->MaxMin[EKF_Quad->ptr2] = RMS;
//		ptr5 = EKF_Quad->ptr2 + 125;
//		if (ptr5 >= 251)
//			ptr5 -= 251;
//		(EKF_Quad->ptr2)++;
//		if (EKF_Quad->ptr2 == 251)
//			EKF_Quad->ptr2 = 0;
//		for (i =0; i < 251 ; i++)														// Shifting the values inside the buffer and finding the maximum and minimum 
//		{
//			if (max < EKF_Quad->MaxMin[i])
//				max = EKF_Quad->MaxMin[i];
//			if (min > EKF_Quad->MaxMin[i])
//				min = EKF_Quad->MaxMin[i];
//		}
//		if (EKF_Quad->MaxMin[ptr5] >  ((max+min)/2+min)/2)
//		{
//			Bneg[4] = -B1_5;
//			Dinv  = 1/(float)(D1);		
//			zeros_2[3] = Dinv;
...
//		}
//		//////////////////////////////
//		Matrix_Multiply((float*)A, EKF_Quad->S, tempMAT_1, 3, 3, 3);						            // tempMAT_1, 3 = A*Sk;
...
//			EKF_Quad->C[i] *= Dinv;
//		RowConcat(Rinv_2, EKF_Quad->C, tempMAT_1, 3, 3, 1);								                  // tempMAT_1 = tempmat[Rb^-1;-D^1*Ck];
...
//		observ = mean - 2*EKF_Quad->X[0]*(EKF_Quad->X[1]*EKF_Quad->X[1])*exp(-1*EKF_Quad->X[1]*EKF_Quad->X[1]);                // observ = y = mean - -2*alpha*d_sigma^2*exp(-d_sigma^2);
//		Matrix_Multiply((float*)A, EKF_Quad->X, tempMAT_2, 3, 3, 1);						           	// tempMAT_2 = A*Xk';
...
//
//		EKF_Quad->X[0] = tempMAT_2[0];														                          // Updating states
//		EKF_Quad->X[1] = tempMAT_2[1];
//		EKF_Quad->X[2] = tempMAT_2[2];
//
//
//		for (i = 0; i < 9 ; i++)												                          // Sk = Ra^-1
//			EKF_Quad->S[i] = Rinv_1[i];
//
//		EKF_Quad->C[0] = exp(-1*EKF_Quad->X[1]*EKF_Quad->X[1]);
//		EKF_Quad->C[1] = -2*EKF_Quad->X[0]*EKF_Quad->X[1]*exp(-1*EKF_Quad->X[1]*EKF_Quad->X[1]);
//		EKF_Quad->C[2] = 1;
//		
//		EKF_Quad->sums =EKF_Quad->sums - EKF_Quad->smoothBuff[EKF_Quad->ps] + EKF_Quad->X[0]*exp(-1*EKF_Quad->X[1]*EKF_Quad->X[1]) + EKF_Quad->X[2];		
//		*Sigma = EKF_Quad->sums / 100;
//		/*dummy = *Sigma;
//		sums_2 = sums_2 - smoothBuff_2[ps_2] + dummy;		
//		*Sigma = sums_2 / 51;*/
//
//		EKF_Quad->smoothBuff[EKF_Quad->ps] = EKF_Quad->X[0]*exp(-1*EKF_Quad->X[1]*EKF_Quad->X[1]) + EKF_Quad->X[2];	
//		//smoothBuff_2[ps_2] = dummy;	
//
//		(EKF_Quad->ps)++;
//		if (EKF_Quad->ps == 100)
//			EKF_Quad->ps = 0;
...
//		if (ps_2 == 51)
//			ps_2 = 0;*/
//		EKF_Quad->delay2[EKF_Quad->p1] = EKF_Quad->RMSBuff[k];																								// Without using buffers
//		*RMS_out = EKF_Quad->delay2[EKF_Quad->p2];																									// Without using buffers
//		
//		(EKF_Quad->p1)++;
//		if (EKF_Quad->p1 == 50)
//			EKF_Quad->p1 = 0;
...
void EKF_quad(float RMS, float *Sigma, float *RMS_out, int init)
{
	...
	static float smoothBuff[150]; //= (float*)(0x10000FA0);
	static float smoothBuff_2[51];
	static float delay2[75]; // = (float*)(0x10001130);
	static int p1 = 0;
	static int p2 = 1;
	static int ps = 0;
	static float sums = 0;
	...
	float mean;
	const float A[9] = {A_1, A_2, A_3, A_4, A_5, A_6, A_7, A_8, A_9};								// A matrix
	float Bneg[9] = {-B1_1, 0, 0, 0, -B1_5, 0, 0, 0, -B1_9};					// -B
	static float S[9]= {Sk_1, Sk_2, Sk_3, Sk_4, Sk_5, Sk_6, Sk_7, Sk_8, Sk_9};								// Sk
	float tempMAT_1[18];
	float tempMAT_2[18];
	float tempMAT_3[18];
	float Q[36];
	...
	float Rinv_1[9];
	float Rinv_2[9];
	static float C[3] = {C0_1 , C0_2 , C0_3};                        // initial value of Ck
	float Dinv  = 1;					 
	static float X[3] = {alpha0, d_sigma0, shift0};								// initial state values
	const float zeros_1[3] = {0,0,0};
	float zeros_2[4] = {0,0,0,1};
	float max = -1000000;
	float min = +1000000;
	...
	if (init == 0)
	{
		...
		ptr3++;
		if (ptr3 == 95)                    									// (95 = (251 - 61) - (251/2 - 51/2);
			ptr3 = 0;
		ptr4++;
		if (ptr4 == 95)
			ptr4 = 0;
		...
		RowConcat(tempMAT_1, (float*)zeros_2, tempMAT_3, 3, 4, 1);					      // tempMAT_3 = [(Rb^-1,zeros(3,3));(zeros(1,3),D^-1)]
		Matrix_Multiply(tempMAT_2, tempMAT_3, tempMAT_1, 3, 4, 4);				        // tempMAT_1 = M
		observ = mean - 2*X[0]*(X[1]*X[1])*exp(-1*X[1]*X[1]);                // observ = y = mean - -2*alpha*d_sigma^2*exp(-d_sigma^2);
		...
		C[0] = exp(-1*X[1]*X[1]);
		C[1] = -2*X[0]*X[1]*exp(-1*X[1]*X[1]);
		C[2] = 1;
		
		sums = sums - smoothBuff[ps] + X[0]*exp(-1*X[1]*X[1]) + X[2];		
		*Sigma = sums / 100;
		/*dummy = *Sigma;
		sums_2 = sums_2 - smoothBuff_2[ps_2] + dummy;		
		*Sigma = sums_2 / 51;*/

		smoothBuff[ps] = X[0]*exp(-1*X[1]*X[1]) + X[2];	
		//smoothBuff_2[ps_2] = dummy;	
		...
		if (ps_2 == 51)
			ps_2 = 0;*/
		delay2[p1] = RMSBuff[k];																								// Without using buffers
		*RMS_out = delay2[p2];																									// Without using buffers
	...
	{
		X[0] = alpha0; X[1] = d_sigma0; X[2] =shift0;
		S[0] = Sk_1; S[1] = Sk_2; S[2] = Sk_3; S[3] = Sk_4; S[4] = Sk_5; S[5] = Sk_6; S[6] = Sk_7; S[7] = Sk_8; S[8] = Sk_9;	
		C[0] = C0_1;  C[1] = C0_2; C[2] = C0_3;	
		p1 = 0;
		p2 = 1;
		ps = 0;
		sums = 0;
...
void EKF_acc(float ACC, float *Sigma, int init)
{
