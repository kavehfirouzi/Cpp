...
#define BUFFER_SIZE 1000																					// Size of the classification window
...
{  
	...
	if (init == 0)																									// If init = 0, run the algorithms
	{
			if (sample_counter > 0)
			{
				...
				PreProcessing((raw.raw_data), &filter_out, &LOW2_out, &detrend_out, 0, 0);		
				...
				if (ptr_2 == BUFFER_SIZE)
					ptr_2 = 0;
				if (smoothedAccBuff[ptr_3] > (max+min)/2)   																		// Classification of accelerometer values
				{
					Class = 1;
				}
				else
				...
				SegmetnAnalysisMVC(Class, smoothedAccBuff[ptr_3], (float*)&intensity, 0, 0);
...
{  
	...
	if (init == 0)																									// If init = 0, run the algorithms
	{
			if (sample_counter > 200)
			{
				...
				PreProcessing((raw.raw_data), &filter_out, &LOW2_out, &detrend_out, 1, 0);	
				/////////////Filtering
				sum_1 = sum_1 - dummy_1[ptr_1] + LOW2_out;
				dummy_1[ptr_1] = LOW2_out;
				if (count_1 < SMOOTH_SIZE-1)
