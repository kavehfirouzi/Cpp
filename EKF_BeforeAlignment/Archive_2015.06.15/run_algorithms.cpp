	  ...
	  for (sel = 0; sel<2; sel++)  																									 // for loop to run Quad and Ham algorithms. sel = 0: Quad, sel = 1: Ham
		{
			...
			PreProcessing((raw.raw_data), &filter_out, &LOW2_out, &detrend_out, sel, 0);
			...
			if (sel == 0) 																																// Calling naive-Bayes segmentation algorithm for Quad
			{
				...
				//*((short*)(0x20003000 + my_counter)) = (short)Sigma;
				//	my_counter += 2;
				f4_1 << Sigma << std::endl;
				Unsupervised(Sigma, &seg_quad, 0);		
				...
				if (cadence_acc == 0) 																											// If cadence is 0, set the intesity and targeting values to 0
				{
				//	Intens_Quad = 0;
				//	Intens_Ham = 0;
				//	targetingQ = 0;
...
//					*((short*)(0x20000000 + my_counter)) = 2500; //(short)Sigma;
				//my_counter += 2;			
				
			
				Unsupervised(Sigma, &seg_ham, 1);	
