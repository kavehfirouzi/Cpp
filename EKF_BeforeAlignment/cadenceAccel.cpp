...
//	float Sigma;
  ...
  static int cadence;
	if (init == 0)																									// If init = 0, run the algorithms
	{
			...
			///////////////////////
			MaxMinBuff[MaxMinPtr] = low;
			for (j=0; j < Accel_size ; j++)														// Shifting the values inside the buffer and finding the maximum and minimum 
			{
		...
		case 1:
			...
			if (in == 0)
			{
				...
				if (peak_counter > 0)																		// Calculate cadence if more than 1 segment have been detected so far
				{
					...
					/*curr = (stop + start)/2;
					if (curr - prev != 0)
						CadAdapt =  7500 / (curr - prev);
					prev = curr;*/
					if (dists[pointer] != 0)
						CadAdapt = 7500  / dists[pointer];
					//f4 << CadAdapt << std::endl;
					pointer++;
					if (pointer == 2)
						pointer = 0;
					if (peak_counter < 2)																	// If only one distance has been detected so far, output it as cadence
